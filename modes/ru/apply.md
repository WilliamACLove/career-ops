...
4. ЗАГРУЗИТЬ    → Прочитать полный отчёт + секцию H (черновики ответов, если есть)
...
4. Если есть секция H → загрузить черновики как базу
...
2. **Секция H**: Если есть черновик — использовать как базу и улучшить
...
Перед тем как кандидат отправит отклик в юрфирму напрямую, проверить поле `legal.recruiter_channel` в `config/profile.yml` и колонку `Via` в трекере: если по этой позиции уже подана (или ведётся) кандидатура через рекрутера/хедхантера — предупредить кандидата о риске referral-source lock (первый подавший "владеет" кандидатурой ~6-12 месяцев; повторная подача может привести к снятию с рассмотрения у обоих каналов) и уточнить, действительно ли он хочет продолжить.
...
2. Обновить секцию с черновиками ответов в отчёте финальными ответами
3. Предложить: `/career-ops contacto` для outreach в LinkedIn/через профильные каналы
